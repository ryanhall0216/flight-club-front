...
export default function Header() {
  ...
  return (
    <header className="bg-black px-4 md:px-12 py-3 flex items-center justify-between max-w-full sticky top-0 z-50 shadow font-regular">
        ...
        <div className="hidden lg:flex gap-4 ml-auto py-3 text-white justify-between w-full">
          <div className="mx-2 px-4 flex items-center">
            <a
              href="#"
              className="flex items-center font-serif capitalize text-[1.125rem] text-white hover:text-red transition-all"
            ...
            >
              ...
              >
                ...
                <span
                  className={
                    `block absolute right-1/2 top-1/2 w-2 h-[2px] bg-white rounded-full origin-right transition-all
                ...
                <span
                  className={
                    `block absolute left-1/2 top-1/2 w-2 h-[2px] bg-white rounded-full origin-left transition-all
                    ${locations ? "rotate-45 -translate-y-1" : "-rotate-45"}`
                  }
                />
              </span>
          ...
          <div className="flex items-center gap-4">
            {navLinks.map((link) => (
              <a
                key={link.label}
                href={link.href}
                className="text-white hover:text-red transition-all capitalize text-[1.125rem] font-brandonRegular"
              >
                {link.label}
              </a>
            ))}
            <div className="flex gap-[16px] px-[16px]">
              <div className="uppercase text-[0.875rem] hover:text-red transition-all lg:leading-3">
                <button
              ...
              <div className="uppercase text-[0.875rem] hover:text-red transition-all lg:leading-3">
                <button
                  className="text-center flex flex-col gap-1 items-center px-4 pt-2 pb-1 bg-green font-semibold text-white uppercase hover:bg-red hover:text-white transition-all"
                  type="button"
                >
